...
function resolveInWorkspace(p) {
  ...
  const w = vscode2.workspace.workspaceFolders?.[0];
  if (!w) return null;
...
async function openLocation(filePath, startLine, endLine) {
  ...
  if (!abs) {
    vscode2.window.showWarningMessage(`File not found in workspace: ${filePath}`);
    return;
  }
  ...
  const start = new vscode2.Position(Math.max((startLine ?? 1) - 1, 0), 0);
  const end = new vscode2.Position(Math.max((endLine ?? startLine ?? 1) - 1, 0), 0);
  editor.revealRange(new vscode2.Range(start, end), vscode2.TextEditorRevealType.InCenter);
  editor.selection = new vscode2.Selection(start, start);
}
...
function itemToNode(it) {
  ...
  const node = new SearchNode(label, vscode3.TreeItemCollapsibleState.None);
  node.command = {
...
var SearchNode = class extends vscode3.TreeItem {
...
function registerSearchOpenCommand(ctx) {
  ctx.subscriptions.push(
    vscode3.commands.registerCommand("rag.openSearchTarget", async (p, s, e) => {
